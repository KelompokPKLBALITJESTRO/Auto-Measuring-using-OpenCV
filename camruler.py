...
def key_event(key):
    ...
    # set ruler
        ...
        key_flags['rulScale'] = not key_flags['rulScale']
...
def mouse_event(event, x, y, flags, parameters):
    ...
    if event == 1:
        ...
        elif key_flags['rulScale']:
            key_flags['rulScale'] = False
            key_flags['config'] = False
            mouse_mark = (ox, oy)
...
while 1:
    ...
    if key_flags['config']:
        ...
        if (key_flags['rulScale'] == False) :
            mysqldb = mysql.connector.connect(host="localhost", user="root", password="", database="balitjestroGrading")
            cursor = mysqldb.cursor()
            cursor.execute("SELECT rullerScale FROM jarakKamera")
            myresult = cursor.fetchall()
            cal_range = myresult[-1][-1]
            text.append(f'Ruler Scale: {cal_range}')

        # clear mouse
        mouse_mark = None

        #Kalibrasi Skala Ruler
        if key_flags['rulScale']:
            key_flags['rulScale'] = False
            root = Tk()
            root.title('Ruler Scale Config')
            root.geometry("350x120")

            Label(root, text="Operator").place(x=10, y=20)
            Label(root, text="Ruler scale").place(x=10, y=50)
    ...
    # -------------------------------
        ...
        for c in contours:
            ...
            # if the contour is too small, ignore it. Auto_percent = 0.2
            if percent < auto_percent:
                continue

            # if the contour is too large, ignore it. Aslinya 60
            elif percent > 6:
                continue
            ...
            xlen = (abs(((x1c-x2c))/10)-0.31)
            ylen = (abs(((y1c-y2c))/10)-0.31)
            alen = 0
            carea=0
            ...
            if key_flags['jeruk']:
                ...
                if (xlen < ylen) :
                    ...
                    elif ((xlen >= 6.10)) & ((xlen <= 7.09)):
                        cv2.putText(frame0, f'Grade: A', (980, 100),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)
                        cv2.putText(frame0, f'R/G: {str(r_ratio)}%', (980, 150),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)

                    elif ((xlen >= 5.10)) & ((xlen <= 6.09)):
                        cv2.putText(frame0, f'Grade: B', (980, 100),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)
                        cv2.putText(frame0, f'R/G: {str(r_ratio)}%', (980, 150),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)

                    elif ((xlen >= 4.10)) & ((xlen <= 5.09)):
                        cv2.putText(frame0, f'Grade: C', (980, 100),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)
                        cv2.putText(frame0, f'R/G: {str(r_ratio)}%', (980, 150),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)
                    ...
                    elif ((ylen >= 6.10)) & ((ylen <= 7.09)):
                        cv2.putText(frame0, f'Grade: A', (980, 100),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)
                        cv2.putText(frame0, f'R/G: {str(r_ratio)}%', (980, 150),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)

                    elif ((ylen >= 5.10)) & ((ylen <= 6.09)):
                        cv2.putText(frame0, f'Grade: B', (980, 100),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)
                        cv2.putText(frame0, f'R/G: {str(r_ratio)}%', (980, 150),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)

                    elif ((ylen >= 4.10)) & ((ylen <= 5.09)):
                        cv2.putText(frame0, f'Grade: C', (980, 100),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)
                        cv2.putText(frame0, f'R/G: {str(r_ratio)}%', (980, 150),
                                    cv2.FONT_HERSHEY_COMPLEX, 1, (128, 0, 0), 2)
    ...
    if key_flags['auto']:
        ...
        text.append(f'S = SET RULER-SCALE')
        text.append('')
